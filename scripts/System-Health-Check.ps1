...
param(
    [int]$CpuThreshold = 80,                    # % CPU (>= alert)
    [int]$MemThreshold = 85,                    # % Memory used (>= alert)
    [int]$DiskFreeThresholdGB = 5,              # Per drive: free space in GB (<= alert)
...
if ($cpu -ge $CpuThreshold) {
    Write-RedAlert "ALERT: CPU usage high: $cpu% (threshold $CpuThreshold%)"
}
...
if ($mem.PctUsed -ge $MemThreshold) {
    Write-RedAlert ("ALERT: Memory usage high: {0}% (threshold {1}%)" -f $mem.PctUsed, $MemThreshold)
}
...
foreach ($d in $disks) {
    ...
    if ($d.FreeGB -le $DiskFreeThresholdGB) {
        Write-RedAlert ("ALERT: Low free space on drive {0}: {1} GB free (threshold {2} GB)" -f $d.Name, $d.FreeGB, $DiskFreeThresholdGB)
    }
}
